---
name: agent-link
description: 'Audit or update `Skills Used` and `References to Prioritize` pointers in agents so runtime adapters stay aligned with the shared core.'
...
- A new skill or reference file changed agent responsibilities
...
2. Compare its skill and reference pointers with the current shared core.
3. Update references while keeping each agent single-purpose.
4. Prefer links to shared references over copied guidance.
