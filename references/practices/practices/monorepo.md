...
- `<root>/.specify/stack.yml` is the **source of truth** for active and disabled AI artifacts (`references`, `skills`, and `agents`) across the monorepo.
- Package-level `stack.yml` files are **derived views** for package-specific context and must never override the root file.
- Any automation that enables, disables, or lists AI artifacts must resolve and update the **root** `stack.yml` first, even when a package `scope` is provided.
- If a package `stack.yml` disagrees with the root, the root wins and the package view should be treated as out of sync until regenerated.
...
| Using a package `stack.yml` to override root artifact state | Conflicting `references` / `skills` / `agents` state across the monorepo | Treat `<root>/.specify/stack.yml` as canonical and regenerate package views from it |
...
- [ ] Root `.specify/stack.yml` is the source of truth for active/disabled `references`, `skills`, and `agents`
